...
def get_text_chunks(text):
    ...
    return chunks
def get_vectorstor(text_chunks) :
    # embeddings = OpenAIEmbeddings()
    # embeddings = HuggingFaceBgeEmbeddings(model_name="EmbeddedLLM/Mistral-7B-Merge-14-v0.2")
...
def main():
    ...
    with st.sidebar:
        ...
        pdf_doc =st.file_uploader("Uplode your PDF's hear and click on process" ,accept_multiple_files= True)
        if st.button("process") :
            with st.spinner("Processing") :
                raw_text = get_pdf_text(pdf_doc)
                text_chunks = get_text_chunks(raw_text)
                vectorstore = get_vectorstor(text_chunks)
